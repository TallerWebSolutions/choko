...
var modelPatch = module.exports = {
  ...
  save: function(data, callback) {
    ...
    this.load(itemKey, function(error, item) {
      ...
      if (item) {
        ...
        var query = {};
        query[self.primaryKey] = itemKey;

        self.update(query, data, function (error, records) {
          if (error) {
            return callback(error);
          }
  ...
  list: function(criteria, callback) {
    var self = this;
    var normalizeCriteria = Object.assign({}, criteria);

    ['where', 'limit', 'sort', 'skip', "populate"].forEach(function (field) {
      if (criteria && criteria[field]) {
        delete normalizeCriteria[field]
      }
    });

    var query = this.find(normalizeCriteria);

    // Extend populate fields.
    query = this.populateQuery(criteria, query);

    ['where', 'limit', 'sort', 'skip'].forEach(function (option) {
      if (criteria && criteria[option]) {
        query = query[option](criteria[option]);
      }
    });
  ...
  load: function(criteria, callback, params) {
    ...
    queryConstructor = this.findOne().where(query);
    queryConstructor = this.populateQuery(params, queryConstructor);
    queryConstructor = this.populateAssociations(params, queryConstructor);

    queryConstructor.exec(function(error, item) {
      if (error) {
        return callback(error);
      }
      ...
      result = self.filterPopulateKeys(result, params);
      ...
      //  }
        ...
        callback(null, result);
        // callback(null, item && item.toObject() || item);
    // });
    });
  },
  ...
  filterPopulateKeys: function (item, params) {
    // Implement PopulateKeys.
    var self = this;
    var associations = [];
    var populateOnlyKeys = true;

    self.associations.forEach(function(association) {
      if (association.type == 'collection') {

        if (params && params.populate) {
          populateOnlyKeys = false;

          if (params.populate.length) {
            populateOnlyKeys = !params.populate[association.alias];
          }
        }

        if (populateOnlyKeys) {
          item[association.alias] = item[association.alias].map(function(rel) {
            return rel[association.keyProperty];
          });
        }
      }
    });
  ...
  populateQuery: function(criteria, query) {
    var DEFAULT_POPULATE_LIMIT = 30;
    var defaultAssociations = this.associations;
    var associations = [];
    var shouldPopulate;

    if (criteria && criteria.populate) {

      // Verify if is an array
      if (criteria.populate.length) {
        defaultAssociations.forEach(function(association) {
          if (criteria.populate) {
            shouldPopulate = lodash.contains(criteria.populate , association.alias);
          }
  ...
  populateAssociations: function(criteria, query) {
    var DEFAULT_POPULATE_LIMIT = 30;
    var populates = criteria && criteria.populate;
    ...
    this.associations.forEach(function(association) {
      if (association.type === 'collection') {
        ...
        else if (populates && populates.length) {
          if (!populates[association.alias]) {
            query.populate(association.alias, {
              limit: association.limit || DEFAULT_POPULATE_LIMIT
            });
          }
