...
Application.prototype.start = function(port, callback) {
  ...
  if (!process.env.NODE_ENV) {
    ...
    this.static(this.settings.applicationDir + '/public/.tmp/' + process.env.NODE_ENV);

    // @TODO: the extensions should have configurations to set available static paths
    // when are executed with defined environments.
    this.static(this.settings.applicationDir + '/public/files');
  }

  var self = this;

  // Allow for custom changes on settings.
  self.emit('applicationSettings', self.settings);
  ...
  function(error, results) {
    ...
    // Exposing port number in settings object.
