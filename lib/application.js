...
Application.prototype.start = function(port, callback) {
  ...
  function(error, results) {
    ...
    // Create application vhost if exist.
    if (self.settings.hostname) {
      self.application.use(vhost(self.settings.hostname, self.application));
    }

    // Adding vhosts routing for application if exist.
    (self.settings.vhosts || []).forEach(function(vhostname) {
      self.application.use(vhost(vhostname, self.application));
    })
...
Application.prototype.buildSettings = function(dir) {
  ...
  self.emit('applicationSettings', settings);
