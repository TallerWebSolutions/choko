...
Storage.prototype.processReferenceField = function(schema, settings, fieldName, fieldSettings) {
  ...
  if ('schema' in field) {
    ...
    // If we set the schema property for a reference field, it will replace the
    // default schema that would be set bellow.
    return utils.extend(schema.attributes[fieldName], this.normalizeSchema(field.schema, fieldSettings));
  }
...
Storage.prototype.processScalarField = function(schema, settings, fieldName, fieldSettings) {
  ...
  utils.extend(attribute, this.normalizeSchema(field.schema, fieldSettings));
...
Storage.prototype.normalizeSchema = function(schema, fieldSettings) {
  ...
  if (schema) {
    switch (typeof schema) {
      ...
      case 'object':
        utils.extend(normalizedSchema, schema);
        break;

      // If schema is a function run it to get the attribute settings
      // object.
      case 'function':
        utils.extend(normalizedSchema, schema(fieldSettings));
        break;
    }
  }
