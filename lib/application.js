    ...
    async.each(Object.keys(self.types), function(typeName, next) {
      ...
      self.list(typeName + 'Type', {}, function(error, subtypes) {
        ...
        async.each(Object.keys(subtypes), function(subtypeName, next) {
          ...
          // Define which settings should be inherited from parent.
          var inheritSettings = utils.merge({
            fields: 'extend'
          }, type.type.settings.inherit, subtypeSettings.inherit);
          ...
          // Iterate over inheriting settings. Avoid deeply merging parent
          // and child, because this could leed to many overriding needs in
          // the resulting settings, as would be the case of the 'polymorphic'
          // attribute.
          // @todo: for now, storage can't be one of the inherited attributes.
          Object.keys(inheritSettings).forEach(function (key) {

            // Allow for children to explicitily avoid inheritance from parent.
            if (inheritSettings[key]) {

              // 'true' means simple override.
              if (inheritSettings[key] === true) {
                inheritSettings[key] = 'override';
              }

              // Switch between inheritance methods.
              switch (inheritSettings[key]) {
                case 'override':
                  subtypeSettings[key] = subtypeSettings[key] || type.type.settings[key] || null;
                  break;
                case 'merge':
                  utils.merge(subtypeSettings[key], type.type.settings[key], subtypeSettings[key]);
                  break;
                case 'extend':
                  subtypeSettings[key] = subtypeSettings[key] || {};
                  utils.extend(subtypeSettings[key], type.type.settings[key] || {});
                  break;
              }
            }
          });
